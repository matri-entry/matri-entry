...
export default function MonitoringCard({ user }: MonitoringCardProps) {
  ...
  } = useCountdown(user.expiryAt);
  const percentage = user.progressPercentage;

  const isExpired = user.isExpired || timerExpired;

  return (
    <Card
      onClick={() => router.push(`/admin/monitoring/${user.id}`)}
      className="rounded-xl border-slate-100 shadow-sm hover-lift overflow-hidden cursor-pointer transition-all hover:shadow-lg"
    >
      {/* Top accent */}
      <div
        className={cn(
        ...
        >
          <Clock
            className={cn(
              ...
              colorClass.replace(" animate-pulse", ""),
            )}
          />
          <span className={cn("text-xs font-semibold", colorClass)}>
            {isExpired ? "Expired" : `${days}d ${hours}h ${minutes}m remaining`}
          </span>
        </div>
      </div>
    </Card>
